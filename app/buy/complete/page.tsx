...
export default async function PurchaseComplete({ searchParams }: { searchParams: Promise<{ audit_id?: string; sale_id?: string; 'custom_fields[audit_id]'?: string; 'url_params[audit_id]'?: string }> }) {
  const params = await searchParams
  const sale_id = params.sale_id
  const finalAuditId = params.audit_id || params['custom_fields[audit_id]'] || params['url_params[audit_id]'] || sale_id || ''
