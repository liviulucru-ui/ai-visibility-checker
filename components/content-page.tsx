...
export function PricingCards({ auditId }: { auditId?: string }) { const plans = [['FREE', '$0', ['Initial visibility check', '8–12 sampled queries', 'Basic visibility result', 'Limited competitor comparison', 'Key findings'], '/check', 'Check My AI Visibility — Free'], ['FULL REPORT', '$19 one-time', ['Full ChatGPT analysis', 'Full Gemini analysis', 'Full Perplexity analysis', 'Full Google AI analysis', 'Complete buyer search evidence', 'Competitor gap analysis', 'Priority recommendations', 'Priority Action Plan', 'Instant PDF export & web report access'], auditId ? `/buy?auditId=${auditId}` : '/buy', 'Get Full Report — $19']] as const; return <div className="grid gap-5 md:grid-cols-2">{plans.map(([name, price, items, href, cta]) => <article key={name} className={`rounded-3xl border p-7 ${name === 'FULL REPORT' ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card'}`}><p className="font-mono text-xs font-bold tracking-[0.2em]">{name}</p><p className="mt-4 font-mono text-4xl font-bold">{price}</p><ul className="my-8 space-y-3 text-sm">{items.map(item => <li key={item} className="flex gap-2"><CheckIcon />{item}</li>)}</ul>{name === 'FULL REPORT' ? (
  <>
